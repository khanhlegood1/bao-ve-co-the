/**
 * affiliate-db.js — IndexedDB lưu hệ thống Affiliate cho Game Portal
 * (bao-ve-co-the). Port thuần JS (không React) từ gameAffiliateDB.js của
 * repo ai-doctor-admin — cùng schema, cùng hành vi, để tương thích 100% với
 * mọi công cụ admin đã có ở repo tham khảo nếu sau này cần đối chiếu dữ liệu.
 *
 * DB name : "cdoc_game_affiliate"
 * Version : 1
 * Stores  :
 *   "codes"     — keyPath: "uuid"                → mã giới thiệu của mỗi user
 *   "referrals" — keyPath: "id", autoIncrement    → quan hệ referrer -> referee
 *                 index: by_referrer, by_referee
 *   "progress"  — keyPath: "id", autoIncrement    → tiến trình chơi từng game
 *                 index: by_uuid, by_game
 *   "rewards"   — keyPath: "id", autoIncrement    → sổ cái thưởng
 *                 index: by_uuid, by_status
 *
 * Mọi ghi thưởng lên blockchain đi qua affiliate-chain.js — file này CHỈ lưu
 * trạng thái local (nguồn sự thật khi offline) + cờ chainStatus để biết dòng
 * nào đã đồng bộ on-chain, dòng nào còn phải gửi lại.
 */
...
// ─── Game progress (từ PORTAL_GAME_RESULT postMessage của mỗi game) ───────
...
// Ghi thưởng cho người chơi (uuid) và, nếu người này là F1 của ai đó (đã vào
// game qua link giới thiệu), tự động cộng thêm hoa hồng cho người giới thiệu
// (referrerUuid) — đây chính là phần "nhận thưởng" của Affiliate.
//
// LƯU Ý ON-CHAIN: contract HienMauAffiliate.rewardTask() đã tự động chia hoa
// hồng lên toàn bộ tuyến trên NGAY TRONG CÙNG 1 giao dịch của người chơi
// (uuid) — dòng "commission" trả về bên dưới CHỈ để hiển thị local, KHÔNG
// được gửi thành 1 giao dịch on-chain riêng. Sau khi giao dịch rewardTask()
// của `primaryId` thành công, gọi markRewardSynced(commissionId, cùng
// txHash đó) — xem affiliate-chain.js.
